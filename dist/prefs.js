...
class PrefBuilder {
    ...
    buildPreview(p) {
        ...
        syncSetting(this.settings, p.settingsKey, () => {
            try {
                w.label = p.format(new Config(this.settings));
                w.get_style_context().remove_class('error');
            }
            catch (e) {
                w.label = 'Error';
    ...
    buildPrefKeybindings(p) {
        ...
        for (const { label, settingsKey } of p.bindings) {
            ...
            let key, mods;
            if (binding) {
                const [success, ...parsed] = Gtk.accelerator_parse(binding);
                if (!success) {
        ...
        {
            const cellrend = new Gtk.CellRendererText();
        ...
        {
            const cellrend = new Gtk.CellRendererAccel({
                editable: true,
            ...
            const col = new Gtk.TreeViewColumn({ title: _('Modify'), min_width: 200 });
...
function buildPrefPages(pages, settings) {
    const builder = new PrefBuilder(settings);
    const notebook = new Gtk.Notebook();
    pages.forEach((p) => {
        const { label } = p;
        if ('rows' in p) {
            notebook.append_page(builder.buildPageRows(p.rows), new Gtk.Label({ label }));
        }
        if ('widget' in p) {
            notebook.append_page(builder.buildPrefKeybindings(p.widget), new Gtk.Label({ label }));
        }
    });
    return notebook;
}
...
function getStoragePrefs() {
    ...
    return [
        ...
        prefRow(_('Preview'), prefPreview(KeyFilenameTemplate, (config) => {
            return get(config.getString(KeyFilenameTemplate), mockDimensions);
        })),
    ];
}
...
class GnomeShellScreenshotSettings extends ExtensionPreferences {
    ...
    fillPreferencesWindow(window) {
        ...
        group.add(buildPrefPages(getPages(), settings));
        page.add(group);
        window.add(page);
...
export default GnomeShellScreenshotSettings;
